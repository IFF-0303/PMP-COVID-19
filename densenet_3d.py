...
def z_score_3d(image_3d):
    mean=image_3d.mean()
    std=image_3d.std()
    image_3d = (image_3d-mean)/(std+1e-6)

    return image_3d


def zero_one_scale_3d(image_3d):
    max=image_3d.max()
    min=image_3d.min()
    image_3d = (image_3d-min)/(max-min+1e-6)

    return image_3d


def normalize_batch_3d(image_3d, norm_type='z-score'):
...
class DenseNet(nn.Module):
    ...
    Args:
        ...
        drop_rate (float) - dropout3d rate after each dense layer
        num_classes (int) - number of classification classes
        small_inputs (bool) - set to True if images are 32x32. Otherwise assumes images are larger.
        efficient (bool) - set to True to use checkpointing. Much more memory efficient, but slower.
    """
    def __init__(self, config, growth_rate=2, block_config=(6, 12, 24, 16), compression=0.5,
                 num_init_features=11, bn_size=4, drop_rate=0,
                 num_classes=2, small_inputs=False, efficient=False):
        ...
        self.ddata_pool = config.ddata_pool
