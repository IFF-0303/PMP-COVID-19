...
def load_checkpoint(path):
    ...
    print('Loading pre-trained models and config file...')
    print(model1_file)
    print(model2_file)
    print(model3_file)
    print(model4_file)
...
def test(model, test_dataloader, config, model_num):
    ...
    with torch.no_grad():
        ...
        print('Testing on dataset3 using pre-trained model %d.' % (model_num))
        ...
        with open('res/%s/test_data3_fold%d_res.pickle' % ('test_res_on_dataset3_'+check_dir, model_num), 'wb') as fp:
...
if __name__ == "__main__":
    ...
    test_data = data_normalize(config,raw_data3, method='test')
